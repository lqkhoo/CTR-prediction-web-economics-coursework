...
training_vectors, n_train, dim_train = _generate_fvs(TRAIN_DATA_PATH, is_training_data = True, domains_filter = clicked_domains) #, limit = 10000)


xs, ys = training_vectors.as_scipy_sparse()

with open('xs_train.dat', 'wb') as outfile:
...
test_vectors, n_test, dim_test = _generate_fvs(TEST_DATA_PATH, is_training_data = False, domains_filter = None, key_filter = training_set_keys) #, limit = 10000)

# Ignore ys
xs, ys = test_vectors.as_scipy_sparse()

with open('xs_test.dat', 'wb') as outfile:
    pickle.dump(xs, outfile, pickle.HIGHEST_PROTOCOL)
